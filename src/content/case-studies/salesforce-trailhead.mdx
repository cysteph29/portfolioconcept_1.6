---
cover: ""
title: "Salesforce Trailhead"
cardTitle: "An AI-Powered Re-Engagement Strategy Trailhead's Power Users Preferred Over ChatGPT"
description: "Salesforce Trailhead's expert users quietly stopped returning once they got certified, taking real work problems to ChatGPT and Slack instead. Over a 10-month capstone, our team researched why power users disengage and designed two connected AI features to bring them back. In testing, 6 of 8 preferred our solution over ChatGPT for Salesforce questions."
problem: "Salesforce's most experienced power users were disengaging, coming back less and less. For a company built on community and advocacy, losing its best advocates is a real problem."
problemEmblem: ""
solution: "A chatbot that answers real Salesforce problems inside Trailhead, paired with a dashboard that turns each visit into a reason to return."
solutionEmblem: ""
outcome: "Given the same Salesforce question, 6 of 8 power users preferred our chatbot's answer over ChatGPT."
outcomeEmblem: ""
heroMosaic: "/assets/salesforce-mosaic.svg"
summaryPattern: "/assets/summary-pattern.svg"
heroVideo: "/work/salesforce-trailhead/video-trailhead-chatbot.mp4"
heroPoster: "/work/salesforce-trailhead/video-trailhead-chatbot-poster.webp"
timeline: "10 months"
role: "ResearchOps, Strategy, AI Prototyping"
team: "Product Designers (team & I), Product Manager"
slug: "salesforce-trailhead"
sections:
  - id: "overview"
    label: "OVERVIEW"
  - id: "problem"
    label: "PROBLEM"
  - id: "research"
    label: "RESEARCH"
  - id: "converge"
    label: "FINDINGS"
  - id: "converge-2"
    label: "IDEATION"
  - id: "ideation"
    label: "Exploration 1"
  - id: "ideation-2"
    label: "Exploration 2"
  - id: "solution"
    label: "THE BET"
  - id: "solution-2"
    label: "SOLUTION"
  - id: "solution-3"
    label: "SOLUTION 01 / SOLVE"
  - id: "solution-4"
    label: "SOLUTION 02 / PROACTIVE PROMPTS"
  - id: "context"
    label: "THE BUILD"
  - id: "context-2"
    label: "Agentic Testing"
  - id: "testing"
    label: "Impact pt.I"
  - id: "testing-2"
    label: "Impact pt.II"
  - id: "reflection"
    label: "MY LEARNING"
---

<Section id="overview" sections={frontmatter.sections}>

## Trailhead is Salesforce's free online learning platform, taking millions of professionals from beginner to certified expert. It was built for the climb, and it loses the experts the moment they reach the top.

Over 10 months, our team of five partnered with the Salesforce Trailhead product team to research, design, and build for the platform's most experienced users: the power users who built their careers on Trailhead and then quietly stopped coming back.

</Section>

<Section id="problem" sections={frontmatter.sections}>

## Salesforce's most valuable power users were disengaging, and for a company built on community and advocacy, that is an expensive thing to lose

Salesforce's ecosystem runs on its community: experienced users who answer questions, mentor newcomers, and champion the platform. Those power users were the engine of it, and they were showing up less and contributing less.

They weren't unhappy users churning, they were quietly drifting. So the problem we set out to solve was re-engagement, not retention: winning back the attention of the platform's most valuable advocates.

</Section>

<Section id="research" sections={frontmatter.sections}>

## I ran research operations end to end, reaching 650+ Salesforce professionals across the US, India, Canada, Europe, and the Middle East, qualifying 72 as power users, and interviewing 12 across two rounds

Round one focused on opinion: how people felt about Trailhead and what would make it more inspiring. It gave us warm, agreeable answers and no real direction.

Round two focused on behavior: what people actually do, where they drop off, whether they use the community and why not, whether certifications even matter to their goals. That is where the real pattern surfaced.

</Section>

<Section id="converge" sections={frontmatter.sections}>

## Power users dropped off right after milestones like certification. And between those moments, they took their questions to ChatGPT and Slack, despite a community on the platform built for exactly those questions.

<CaseFigure
  src="/work/salesforce-trailhead/case-study-trailhead-5.webp"
  alt="Graph comparing Trailhead engagement against the learner journey from certification to real-world problem solving"
  caption="Engagement drops off exactly where the certification path ends."
  width={1920}
  height={1080}
/>

<CasePercentageCards
  label="Research findings from Trailhead power users"
  items={[
    {
      label: "Disengaged after major milestones like certifications",
      valueLabel: "64%",
    },
    {
      label: "Supplemented Trailhead with ChatGPT, YouTube, and Slack",
      valueLabel: "45%",
    },
    {
      label: "Found the content too shallow for their experience",
      valueLabel: "45%",
    },
    {
      label: "Were unhappy with the existing personalization",
      valueLabel: "36%",
    },
  ]}
/>

</Section>

<Section id="converge-2" sections={frontmatter.sections}>

## We explored a range of directions, and the two we killed taught us the most

Two ideas got far enough that we designed them fully before deciding not to build them. What we learned by killing them is what shaped the two features we shipped.

<CaseFigure
  src="/work/salesforce-trailhead/case-study-trailhead-1.webp"
  alt="Whiteboard synthesis with affinity mapping and concept sketches for Trailhead engagement directions"
  caption="Whiteboard synthesis, affinity mapping, concept sketches"
  width={1920}
  height={1080}
/>

</Section>

<Section id="ideation" sections={frontmatter.sections}>

## We almost gave power users an AI version of themselves to run their community presence. We killed it to protect Trailhead's trust.

The idea was status plus leverage: power users become group leaders, and an AI trained on their posts answers community questions on their behalf.

We took it down two paths, and both broke. Fully automated meant undisclosed AI speaking as a real person, which damages trust. Disclosed or approve-first clashed with Trailhead's warm, human brand and the real anti-AI anxiety among users, and the approval version stopped saving anyone time, which was the entire point. Underneath all of it, our research was clear that these users didn't want to engage with the community anyway.

The premise was shaky, so we let it go. But we kept one thing: people wanted fast, expert answers. We just had to strip away the fake human.

</Section>

<Section id="ideation-2" sections={frontmatter.sections}>

## We designed a public skill profile meant to pressure power users into staying current, then killed it because it only worked if employers bought in

The idea borrowed from GitHub's contribution graph. Make someone's skill currency public, and the visibility itself pressures them to keep it up. A shareable Trailhead profile would show certifications, badges, and skill levels tied to how current your learning is, replacing the scattered cert posts people drop on LinkedIn. Keep it public, and people come back to keep it current.

We killed it for two reasons. First, it only works if the outside world treats the profile as a credential, and that adoption was entirely outside our control. Second, even if it caught on, it would drive engagement for the sake of engagement, not the meaningful, lasting reason to return we were after. We carried its skill-visibility idea into the dashboard instead.

<CaseFigure
  src="/work/salesforce-trailhead/case-study-trailhead-7.webp"
  alt="Public skill profile concept UI"
  caption="The shareable Trailhead profile showing certifications, badges, and skill currency, the idea we killed."
  width={2400}
  height={4165}
/>

</Section>

<Section id="solution" sections={frontmatter.sections}>

## We bet that plain-language answers drawn from Salesforce's own data would beat any general AI tool. We never got that access, so we proved the experience instead.

ChatGPT runs on public data. Slack answers depend on whoever happens to reply. We bet that Salesforce's proprietary knowledge could beat both.

Access to that data, and to Agentforce, was never on the table, so we built on OpenAI with curated Salesforce documentation and focused on what we could prove: the experience.

</Section>

<Section id="solution-2" sections={frontmatter.sections}>

## We shipped two connected experiences: answers the moment a problem hits, and a system that surfaces what's relevant next without being asked

Solve catches power users at the moment they would otherwise open ChatGPT. The second works in the background, resurfacing relevant next steps so returning is worth it. Neither works alone.

</Section>

<Section id="solution-3" sections={frontmatter.sections}>

## Solve answers a real Salesforce problem in plain language, on the spot, calibrated to your expertise and backed by sources you can check

<CaseVideo
  src="/work/salesforce-trailhead/video-trailhead-chatbot.mp4"
  poster="/work/salesforce-trailhead/video-trailhead-chatbot-poster.webp"
  alt="Salesforce Trailhead AI chatbot walkthrough"
  caption="Solve answering"
  width={1660}
  height={1080}
/>

**How it works:**
* Ask a problem in plain language and get a specific, actionable answer that cites Salesforce documentation.
* Set the depth to Beginner, Intermediate, or Advanced, so you are never over-explained or underserved.
* When Solve can't fully help, it drafts a ready-to-post Trailblazer Community question from your conversation, so you get an answer either way.

That last one mattered to us. We designed for the moment the AI can't help, not just the moment it can.

</Section>

<Section id="solution-4" sections={frontmatter.sections}>

## Without being asked, the system surfaces what's relevant to each expert: skill gaps against the market, follow-ups from their questions, and renewals before they lapse

<CaseFigure
  src="/work/salesforce-trailhead/case-study-trailhead-4.webp"
  alt="Personalised Dashboard full view showing trending skills and personalized trail recommendations"
  caption="Dashboard full view"
  width={1920}
  height={3012}
/>

**How it works:**
* Before a certification lapses, it flags the renewal on its own, closing a notification gap users complained about.
* Unprompted, it measures a user's skill profile against what the market is hiring for and builds a path to close the gap it finds.
* It reads the questions they asked Solve and recommends what to learn next, so the follow-up finds them.

None of this waits for a request. The platform does the noticing and brings the next step to the user.

</Section>

<Section id="context" sections={frontmatter.sections}>

## Figma MCP took each component straight into Cursor and Claude Code, where we wired it to the OpenAI API and shipped Solve as a working app, not a mockup

<CaseVideo
  src="/work/salesforce-trailhead/video-trailhead-chatbot.mp4"
  poster="/work/salesforce-trailhead/video-trailhead-chatbot-poster.webp"
  alt="Trailhead Solve prototype in action"
  caption="The build / prototype in action"
  width={1660}
  height={1080}
/>

After designing each component in Figma against Trailhead's design system, we built it one piece at a time, no handoff, no rebuild from a spec. The result was a functional product: a React front end, an Express and SQLite back end, OpenAI for responses, voice input, expertise calibration, and community-post drafting, all running.

Taking a concept from design to working software ourselves is what let us test it like a real product instead of a slideshow.

</Section>

<Section id="context-2" sections={frontmatter.sections}>

## Real power users were hard to get hold of, so we built 12 AI agents from our research data to catch the obvious problems before spending anyone's real time

We turned our research into 12 AI agents and ran them through the flows to surface usability problems first. By the time real users arrived, the rough edges were already smoothed, and their time went to what actually needed a person.

</Section>

<Section id="testing" sections={frontmatter.sections}>

## We tested whether Solve could replace the reflex to open ChatGPT. Given the same Salesforce question, 6 of 8 power users preferred Solve.

Each participant took a question about the Salesforce product they work with, asked it to both ChatGPT and Solve, and compared.

<CaseMetrics
  label="Solve comparison test results"
  variant="scores"
  items={[
    {
      label: "Preferred Solve over ChatGPT",
      value: 6,
      valueLabel: "6/8",
      max: 8,
    },
    {
      label: "Confidence in Solve's answers",
      value: 4.5,
      valueLabel: "4.5/5",
      max: 5,
    },
  ]}
/>

</Section>

<Section id="testing-2" sections={frontmatter.sections}>

## The prototype pulled power users back into the content, lifting session duration 35% and cutting drop-off 22%

This was the half of the problem we most needed to prove: that experts who used to leave after a milestone would stay.

<CaseMetrics
  label="Prototype engagement impact"
  variant="deltas"
  items={[
    {
      label: "Session duration",
      value: 35,
      valueLabel: "+35%",
    },
    {
      label: "Drop-off",
      value: -22,
      valueLabel: "−22%",
    },
  ]}
/>

</Section>

<Section id="reflection" sections={frontmatter.sections}>

## I had never been comfortable in code, and had never prototyped with it, so teaching myself to ship this as working software is the barrier I'm proudest of clearing

For most of my career, code was something I designed around. I had never prototyped with it and never felt at home in it. This project left me no room to avoid it, and using AI coding agents to build and ship a working product turned that limitation into something I can now do.

</Section>
